...
def categorize_top_series(session, SERVER_ID, BASE_URL):
    try:
        ...
        # Helper function to normalize serie names
        def normalize_name(name):
            return name.strip().lower()

        # Function to match tomatoes against fetched series
        def match_series(remote_map, tomatoes):
            matches = []
        ...
        # Match series for each service
        print("Matching tomatoes with remote vod...")
        netflix_matches = match_series(remote_serie_map, netflix_tomatoes)
        print(f"Netflix Matches ({len(netflix_matches)})")
        prime_matches = match_series(remote_serie_map, prime_tomatoes)
        print(f"Prime Video Matches ({len(prime_matches)})")
        disney_matches = match_series(remote_serie_map, disney_tomatoes)
            ...
            if matched_category:
                ...
                try:
                    ...
                    if delete_response.status_code == 200:
                        print(f"✅ Deleted category '{target_name}'  successfully.")
                    else:
                        print(
        ...
        # Step: Add matched series to their respective categories
        category_matches = {
        ...
        for cat_name, matches in category_matches.items():
            matched_serie_ids = [str(item["id"]) for item in matches]
            if matched_serie_ids:
                category_id = category_map.get(cat_name.lower())
                if category_id:
                    payload = {
                        "action": "edit",
                        "category_id": category_id,
                        "list": ",".join(matched_serie_ids) + ",",
                    }
                    try:
                        res = session.post(
                        ...
                        res.raise_for_status()
                        print(
                            f"✅ Category '{cat_name}' (ID: {category_id}) updated with {len(matched_serie_ids)} series"
                        )
                    except requests.RequestException as e:
                ...
                print(f"⚠️ No matched series for '{cat_name}'.")
